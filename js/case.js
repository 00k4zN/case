(() => {
    ...
    const createProductCard = (product, index) => {
        try {
            ...
            card.innerHTML = `
            <div class="eb-product-link-container">
    ...
    const buildHTML = () => {
        ...
        bannerWrapper.className = 'banner_wrapper';
        ...
        productShelf.style.width = (products.length * 244) + 'px';
        ...
        // Banner wrapper'a carousel container'ı ekliyoruz
        ...
        if (bannerTitles) {
            bannerTitles.parentNode.insertBefore(bannerWrapper, bannerTitles);
    ...
    const buildCSS = () => {
        ...
        .eb-owl-stage {
            ...
            padding: 20px 10px; /* Yatay padding azaltıldı */
        }
        
        .eb-owl-item {
            flex-shrink: 0;
            padding: 8px 0;
            margin-right: 16px; /* 20px'ten 16px'e düşürüldü */
            width: 244px; 
            height: 520px;
            transition: all 0.3s ease;
        }
        
        /* Son öğenin margin-right değerini kaldırmak önemli */
        .eb-owl-item:last-child {
            margin-right: 0;
        }
        ...
        .eb-favorite-button {
            ...
            cursor: pointer;
            z-index: 3; /* z-index değerini arttırdık */
            padding: 0;
    ...
    const setupEvents = () => {
        ...
        const productCards = document.querySelectorAll('.eb-product-card');
        productCards.forEach(card => {
            // Kart içindeki tüm tıklamalar için olay dinleyici
            card.addEventListener('click', (e) => {
                // Eğer tıklanan eleman butonlardan biriyse, kart linki çalışmasın
                if (e.target.closest('.eb-favorite-button') || e.target.closest('.eb-add-to-cart-button')) {
                    e.stopPropagation();
                }
            });
        });
